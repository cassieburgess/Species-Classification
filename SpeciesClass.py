    ...
    for k in range(0, height -n, shiftSize): 
        for m in range(0, width - n, shiftSize): 
          ...
          #  currentSpecies, currentProb = oneSpeciesOverlap((m,k), n, imageName, overlap, subTileDict, fit, scaler)
            currentSpecies = oneSpeciesOverlap((m,k), n, imageName, overlap, subTileDict, fit, scaler)
...
def classifyMap(classifier, densityList, metricList,scaler,imageName):
    Species = allSpeciesOverlap(50, imageName, 0.2, densityList, metricList, classifier, scaler)
  #  print(Species)
    SpeciesMapShort(Species, imageName, 0.2, 50) 
    return